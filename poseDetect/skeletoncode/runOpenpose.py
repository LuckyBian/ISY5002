...
from poseDetect.skeletoncode.utils.contrastImg import coincide
import glob
import time
...
def run_demo(net, image_provider, height_size, cpu, boxList):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for img in image_provider:  # 遍历图像集
        ...
        fallFlag = 0
        if i % 1 == 0:
            # 返回热图,paf,输入模型图象相比原始图像缩放倍数,输入模型图像padding尺寸
            ...
            # print("Detected keypoints:", total_keypoints_num)
            # print("Keypoints confidence:", all_keypoints_by_type)
            ...
            for pose in current_poses:
                ...
                # cv2.imshow('skeleton image', pose.img_pose)
                #
                # cv2.waitKey(1)
                ...
                #     cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]), (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (0, 0, 255), thickness=3)
                #     cv2.putText(img, 'state: {}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255))
                #     fallFlag = 1
                # else:
                #     cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]), (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (0, 255, 0), thickness=5)
                #     cv2.putText(img, 'state: {}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0))
                #     fallFlag = 1
            # if fallFlag == 1:
            #     t = time.time()
            ...
            # t = time.time()
            # t = int(round(t * 1000))
            # cv2.imwrite(f'C:\\Users\\eason\\Desktop\\ism_person_openpose-master\\test\\pose\\{t}.jpg', img)
...
def get_video_files(directory):
    video_files = []
    for file in glob.glob(os.path.join(directory, '*.mp4')):  # 根据实际视频文件格式修改匹配规则
