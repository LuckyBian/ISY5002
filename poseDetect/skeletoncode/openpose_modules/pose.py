...
import numpy as np
import os
from poseDetect.skeletoncode.openpose_modules.keypoints import BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS
from poseDetect.skeletoncode.openpose_modules.one_euro_filter import OneEuroFilter
import time
    ...
    def draw(self, img,is_save = False,show_draw = True):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):   # 将躯干某个连接的单位向量映射到paf对应的通道
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:  # 起点和终点均分配
                ...
                cv2.line(I, (int(px_a*scale), int(py_a*scale)), (int(px_b*scale), int(py_b*scale)),[255, 255, 255], 8)
            #**************************************
        # 保存骨骼图片
        if is_save:
            save_dir = 'poseDetect'  # 文件夹名称
            file_name = 'skp.jpg'  # 固定文件名
